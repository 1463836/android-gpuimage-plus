...
namespace CGE {
    ...
    public:
        ...
        static bool
        mergeCurve(std::vector<float> &dst, std::vector<float> &late, std::vector<float> &early);
        ...
        static bool
        genCurve(std::vector<CurveData> &vec, const CurvePoint *pnts, size_t cnt, size_t channel);

        static bool mergeCurveConst(std::vector<CurveData> &dst, const std::vector<CurveData> &late,
                                    const std::vector<CurveData> &early);

        //Allow mergeCurve to scale the early and late to the same length.
        static bool mergeCurve(std::vector<CurveData> &dst, std::vector<CurveData> &late,
                               std::vector<CurveData> &early);

        static bool mergeCurve(std::vector<CurveData> &dst, std::vector<float> &late,
        ...
        static bool
        loadCurve(std::vector<CurveData> &vec, const float *curve, size_t cnt, size_t dstChannel,
    ...
    protected:
        ...
        //stride: 0,1,2
        static bool
        _genCurve(float *curve, const CurvePoint *pnts, size_t cnt, unsigned channel = 1,
                  unsigned stride = 0);

        static void _assignCurveArrays(ProgramObject &program, CGEConstString name,
                                       std::vector<CurveData> &data);

        static void _assignCurveSampler(GLuint &texID, std::vector<CurveData> &data);
    };

    class CGECurveTexFilter : public ImageFilterInterface, public CGECurveInterface {
    public:
    ...
    class CGECurveFilter : public ImageFilterInterface, public CGECurveInterface {
    public:
        CGECurveFilter() {}
    ...
    class CGEMoreCurveFilter : public ImageFilterInterface, public CGECurveInterface {
    public:
        CGEMoreCurveFilter() {}
