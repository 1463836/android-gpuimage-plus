...
namespace CGE {
    ...
    struct Mat4 {
        ...
        static inline Mat4
        makeFrustum(float left, float right, float bottom, float top, float nearZ, float farZ) {
        ...
        static inline Mat4
        makeOrtho(float left, float right, float bottom, float top, float nearZ, float farZ) {
            const float ral = right + left;
            const float rsl = right - left;
            const float tsb = top - bottom;
            const float tab = top + bottom;
        ...
        static inline Mat4
        makeLookAt(float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ,
        ...
        static inline bool
        projectM4fPerspective(const Vec3f &obj, const Mat4 &modelView, const Mat4 &proj,
                              const Vec4f &viewPort, Vec3f &winCoord) {
            //Transformation vectors
            float fTempo[8];
            //modelView transform
            fTempo[0] =
                    modelView[0][0] * obj[0] + modelView[1][0] * obj[1] + modelView[2][0] * obj[2] +
                    modelView[3][0];  //w is always 1
            fTempo[1] =
                    modelView[0][1] * obj[0] + modelView[1][1] * obj[1] + modelView[2][1] * obj[2] +
                    modelView[3][1];
            fTempo[2] =
                    modelView[0][2] * obj[0] + modelView[1][2] * obj[1] + modelView[2][2] * obj[2] +
                    modelView[3][2];
            fTempo[3] =
                    modelView[0][3] * obj[0] + modelView[1][3] * obj[1] + modelView[2][3] * obj[2] +
                    modelView[3][3];
            //proj transform, the final row of proj matrix is always [0.0, -1.0]
            //so we optimize for that.
            fTempo[4] = proj[0][0] * fTempo[0] + proj[1][0] * fTempo[1] + proj[2][0] * fTempo[2] +
                        proj[3][0] * fTempo[3];
            fTempo[5] = proj[0][1] * fTempo[0] + proj[1][1] * fTempo[1] + proj[2][1] * fTempo[2] +
                        proj[3][1] * fTempo[3];
            fTempo[6] = proj[0][2] * fTempo[0] + proj[1][2] * fTempo[1] + proj[2][2] * fTempo[2] +
                        proj[3][2] * fTempo[3];
            fTempo[7] = -fTempo[2];
        ...
        static inline bool
        projectM4f(const Vec3f &obj, const Mat4 &modelView, const Mat4 &proj, const Vec4f &viewport,
                   Vec3f &winCoord) {
        ...
        static inline bool
        projectM4f(const Vec2f &obj, const Mat4 &modelView, const Mat4 &proj, const Vec4f &viewport,
                   Vec2f &winCoord) {
        ...
        static inline bool
        projectM4f(const Vec3f &obj, const Mat4 &modelViewProjection, const Vec4f &viewport,
                   Vec3f &winCoord) {
        ...
        static inline bool
        projectM4f(const Vec3f &obj, const Mat4 &modelView, const Mat4 &proj, Vec2f &winCoord,
        ...
        static inline bool
        projectM4f(const Vec3f &obj, const Mat4 &mvp, Vec2f &winCoord, float width = 1.0f,
                   float height = 1.0f) {
        ...
        inline Mat4 operator*(const Mat4 &m) const {
            ...
            return Mat4(data[0][0] * m[0][0] + data[1][0] * m[0][1] + data[2][0] * m[0][2] +
                        ...
                        data[3][3] * m[2][3],
                        data[0][0] * m[3][0] + data[1][0] * m[3][1] + data[2][0] * m[3][2] +
                        data[3][0] * m[3][3],
                        data[0][1] * m[3][0] + data[1][1] * m[3][1] + data[2][1] * m[3][2] +
                        data[3][1] * m[3][3],
                        data[0][2] * m[3][0] + data[1][2] * m[3][1] + data[2][2] * m[3][2] +
                        data[3][2] * m[3][3],
                        data[0][3] * m[3][0] + data[1][3] * m[3][1] + data[2][3] * m[3][2] +
                        data[3][3] * m[3][3]);
        }
