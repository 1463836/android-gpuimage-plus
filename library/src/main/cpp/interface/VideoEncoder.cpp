...
static AVStream *addStream(AVFormatContext *oc, AVCodec **codec,
                           ...
                           int bitRate = 1650000, int audioSampleRate = 44100) {
    AVCodecContext *c;
    AVStream *st;
...
namespace CGE {
    struct CGEEncoderContextMP4 {
        CGEEncoderContextMP4() : pOutputFmt(nullptr), pFormatCtx(nullptr), pVideoStream(nullptr),
                                 pAudioStream(nullptr), pVideoCodec(nullptr), pAudioCodec(nullptr),
                                 pVideoFrame(nullptr), pAudioFrame(nullptr), pSwsCtx(nullptr),
                                 pSwrCtx(nullptr), dstSampleData(nullptr), dstSamplesSize(0),
    ...
    bool
    CGEVideoEncoderMP4::init(const char *filename, int fps, int width, int height, bool hasAudio,
                             int bitRate, int audioSampleRate, AVDictionary *options,
                             const char *rotation) {
        m_hasAudio = hasAudio;
        ...
        // loge("ret num %x, filename %s", ret, filename);
        ...
        if (m_context->pOutputFmt->video_codec != AV_CODEC_ID_NONE) {
            m_context->pVideoStream = addStream(m_context->pFormatCtx, &m_context->pVideoCodec,
                                                ...
                                                height, bitRate, audioSampleRate);
        ...
        if (0 >
            avformat_write_header(m_context->pFormatCtx, options == nullptr ? nullptr : &options)) {
            loge("avformat_write_header failed...");
            return false;
        }

        logi("avformat_write_header OK!");
    ...
    bool CGEVideoEncoderMP4::record(const ImageData &data) {
            ...
            if (pixFmt != codecCtx->pix_fmt || data.width != codecCtx->width ||
                ...
                if (!m_context->pSwsCtx) {
                    m_context->pSwsCtx = sws_getContext(data.width, data.height, pixFmt,
                                                        ...
                                                        nullptr, nullptr);
                    if (!m_context->pSwsCtx) {
                        loge("create sws context failed!");
                        return false;
                    }
                    logi("create sws context success!");
                }
        ...
        if (m_context->pFormatCtx->oformat->flags & AVFMT_RAWPICTURE) {
            ...
            if (0 > ret) {
                loge("av_interleaved_write_frame error1...");
                return false;
            }
            av_free_packet(&pkt);
        } else {
            ...
            if (gotPacket && pkt.size) {
                ...
                if (0 > ret) {
                    loge("av_interleaved_write_frame error2... 0x%x\n", ret);
                    return false;
                }
    ...
    bool CGEVideoEncoderMP4::recordVideoFrame(AVFrame *pVideoFrame) {
        ...
        if (gotPacket && pkt.size) {
            ...
            // tm2 = getCurrentTimeMillis();
            // loge("av_interleaved_write_frame 花费时间: %g", (tm2 - tm));
            // tm = tm2;
    ...
    int CGEVideoEncoderMP4::getAudioSampleRate() {
        return (m_context && m_context->pAudioStream) ? m_context->pAudioStream->codec->sample_rate
                                                      : 0;
    }
}
