...
GLuint cgeGlobalTextureLoadFunc(const char *sourceName, GLint *w, GLint *h, void *arg) {
    ...
    jmethodID loadTextureMethod = env->GetStaticMethodID(cls, "loadTextureByName",
                                                         "(Ljava/lang/String;)Lorg/wysaid/nativePort/CGENativeLibrary$TextureResult;");

    if (loadTextureMethod == nullptr) {
        loge("Fatal error: find method failed!\n");
        return 0;
    }
    ...
    logi(
            "cgeGlobalTextureLoadFunc loadImage success - srcName: %s, texID: %d, texWidth: %d, texHeight: %d",
            sourceName, texID, width, height);
...
CGETextureResult cgeLoadTexFromBitmap_JNI(JNIEnv *env, jclass cls, jobject bitmap) {
    ...
    jmethodID loadTextureMethod = env->GetStaticMethodID(cls, "loadTextureByBitmap",
                                                         "(Landroid/graphics/Bitmap;)Lorg/wysaid/nativePort/CGENativeLibrary$TextureResult;");
...
CGETextureResult cgeLoadTexFromFile_JNI(JNIEnv *env, jclass cls, jstring imgFile) {
    ...
    jmethodID loadTextureMethod = env->GetStaticMethodID(cls, "loadTextureByFile",
                                                         "(Ljava/lang/String;)Lorg/wysaid/nativePort/CGENativeLibrary$TextureResult;");
