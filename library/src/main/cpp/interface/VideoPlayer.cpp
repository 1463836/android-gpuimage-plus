...
namespace CGE
{
    ...
    {
        program.bindAttribLocation(ImageFilterInterface::paramPositionIndexName, m_posAttribLocation);

        if(!program.initWithShaderStrings(s_vshYUV420p, s_fshYUV420p))
        {
            loge("cgeVideoPlayerYUV420P program init failed!");
            return;
        }

        program.bind();
        program.sendUniformMat3("m3ColorConversion", 1, GL_FALSE, s_colorConversion601);

        //textureY

        m_texYLoc = program.uniformLocation("textureY");
        m_texULoc = program.uniformLocation("textureU");
        m_texVLoc = program.uniformLocation("textureV");
        ...
        {
            loge("Invalid YUV Texture Uniforms\n");
        }
    ...
    bool CGEVideoPlayerYUV420P::initWithDecodeHandler(CGEVideoDecodeHandler* handler)
    {
        ...
        logi("CGEVideoPlayerYUV420P vertex buffer id: %d", m_vertexBuffer);
    ...
    void CGEVideoPlayerYUV420P::render()
    {
        ...
        if(m_texYUV[0] == 0)
        {
            loge("You should get a frame first!\n");
            return ;
        }
        );
    ...
    void CGEVideoPlayerYUV420P::setRotation(float rad)
    {
        ...
        program.bind();
        glUniformMatrix2fv(locationRotation, 1, GL_FALSE, mat2);
    }
