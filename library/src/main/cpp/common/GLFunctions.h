...
#define CGE_ENABLE_GLOBAL_GLCONTEXT(...) enableGlobalGLContext()
...
namespace CGE {
    ...
    typedef bool (*EnableGLContextFunction)(void *);

    typedef bool (*DisableGLContextFunction)(void *);
    ...
    typedef void *(*CGEBufferLoadFun)(const char *sourceName, void **bufferData, GLint *w, GLint *h,
                                      CGEBufferFormat *fmt, void *arg);
    ...
    void *cgeLoadResourceCommon(const char *sourceName, void **bufferData, GLint *w, GLint *h,
    ...
    inline GLint getMaxTextureSize() {
        GLint n;
    ...
    class SharedTexture {
    public:
        SharedTexture(int w = 0, int h = 0) : m_textureID(0), m_refCount(nullptr), width(w),
                                              height(h) {}
        ...
        inline SharedTexture &operator=(const SharedTexture &other) {
            ...
            if (m_refCount) {
                ++*m_refCount;
                logi("CGESharedTexture assgin: textureID %d, refCount: %d\n", m_textureID,
                     *m_refCount);
            }
    ...
    public:
        ...
        inline void bindTexture2D(const SharedTexture &texture,
                                  GLenum attachment = GL_COLOR_ATTACHMENT0) const {
            bindTexture2D(texture.texID(), texture.width, texture.height, attachment);
        }

        inline void
        bindTexture2D(const SharedTexture &texture, GLsizei x, GLsizei y, GLsizei w, GLsizei h,
                      GLenum attachment = GL_COLOR_ATTACHMENT0) const {
            bindTexture2D(texture.texID(), x, y, w, h, attachment);
        }

        inline void bindTexture2D(GLuint texID, GLsizei w, GLsizei h,
                                  GLenum attachment = GL_COLOR_ATTACHMENT0) const {
        ...
        inline void bindTexture2D(GLuint texID, GLenum attachment = GL_COLOR_ATTACHMENT0) const {
            ...
            (
                    ...
                    if (code != GL_FRAMEBUFFER_COMPLETE) {
                        loge(
                                "CGE::FrameBuffer::bindTexture2D - Frame buffer is not valid: %x\n",
                                code);
                    }
            )
        }
