...
bool _cgeCheckGLError(const char *name, const char *file, int line) {
    ...
    for (GLenum error = glGetError(); loopCnt < 32 && error; error = glGetError(), ++loopCnt) {
        ...
        loge("After \"%s\" glGetError %s(0x%x) at %s:%d\n", name, pMsg, error, file, line);
    }
...
const char *cgeGetBlendModeName(CGETextureBlendMode mode, bool withChinese) {
    ...
    static ConstString s_blendModeName[] =
            {
                    "mix", // 0
                    "dissolve", // 1
    ...
    static ConstString s_blendModeNameWithChinese[] =
            {
                    "mix(正常)", // 0
                    "dissolve(溶解)", // 1
...
void getDataAndChannelByFormat(CGEBufferFormat fmt, GLenum *dataFmt, GLenum *channelFmt,
                               GLint *channel) {
    GLenum df, cf;
    GLint c;
    switch (fmt) {
...
//GLint channels = 4, GLint bindID = 0, GLenum texFilter = GL_LINEAR, GLenum texWrap = GL_CLAMP_TO_EDGE
GLuint
genTextureWithBuffer(const void *bufferData, GLint dstWidth, GLint dstHeight, GLenum channelFmt,
                     GLenum dataFmt,
    ...
    if (channels <= 0 || channels > 4)
        return 0;
    const GLenum &internalFormat = eArrs[channels - 1];
    glActiveTexture(GL_TEXTURE0 + bindID);
    glGenTextures(1, &tex);
    glBindTexture(GL_TEXTURE_2D, tex);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, dstWidth, dstHeight, 0, channelFmt, dataFmt,
                 bufferData);
