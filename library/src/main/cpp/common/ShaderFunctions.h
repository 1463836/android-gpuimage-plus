...
namespace CGE {
    ...
    public:
        ...
        void assignUniforms(ImageHandlerInterface *handler, GLuint programID);
    ...
    public:
        ...
        bool
        linkWithShaderObject(ShaderObject &vertObj, ShaderObject &fragObj, bool shouldClear = true);

        inline bool link() {
            return linkWithShaderObject(vertObj, fragmentObj);
        }
        ...
        inline void sendUniformf(const char *name, GLfloat x) {
            GLint uniform = _getUniform(programID, name);
        ...
        inline void
        sendUniformMat2(const char *name, int count, GLboolean transpose, const GLfloat *matrix) {
            GLint uniform = _getUniform(programID, name);
            glUniformMatrix2fv(uniform, count, transpose, matrix);
        }

        inline void sendUniformMat3(const char *name, GLsizei count, GLboolean transpose,
                                    const GLfloat *matrix) {
            GLint uniform = _getUniform(programID, name);
            glUniformMatrix3fv(uniform, count, transpose, matrix);
        }

        inline void sendUniformMat4(const char *name, GLsizei count, GLboolean transpose,
                                    const GLfloat *matrix) {
            GLint uniform = _getUniform(programID, name);
        ...
        inline GLint attributeLocation(const char *name) {
        ...
        inline void bindAttribLocation(const char *name, GLuint index) {
    ...
    private:
        ShaderObject vertObj, fragmentObj;
        GLuint programID;
    };
