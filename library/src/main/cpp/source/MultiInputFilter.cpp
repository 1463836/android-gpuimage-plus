...
#include "MultiInputFilter.h"

namespace CGE {
    CGEMultiInputFilter::~CGEMultiInputFilter() {
        logi("CGEMultiInputFilter::~CGEMultiInputFilter : %p", this);
    }

    void CGEMultiInputFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
        ...
        if (m_inputTextures.empty()) {
            loge("CGEMultiInputFilter::render2Texture - input textures are empty!");
        } else {
        ...
        if (m_inputTextureLocations.size() != m_inputTextures.size()) {
            ...
            for (int i = 0; i != m_inputTextureLocations.size(); ++i) {
                ...
                if (m_inputTextureLocations[i] < 0) {
                    loge("Invalid uniform name %s!!", buffer);
                }
            }
        }
    }
...
JNIEXPORT jlong
JNICALL Java_org_wysaid_nativePort_CGEMultiInputFilterWrapper_nativeCreate
...
Java_org_wysaid_nativePort_CGEMultiInputFilterWrapper_nativeRelease
...
JNIEXPORT void JNICALL
Java_org_wysaid_nativePort_CGEMultiInputFilterWrapper_nativeUpdateInputTextures
        (JNIEnv
         *env,
         jobject cls, jlong
         addr,
