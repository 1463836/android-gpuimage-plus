...
extern "C"
{
    ...
    {
        logi("##### nativeGenerateVideoWithFilter!!!");
...
namespace CGE
{
    ...
    public:
        ...
        void processingFilters()
        {
            ...
            for(auto& filter : vecFilters)
            {
                ...
                filter->render2Texture(this, fboTextures[1], vertexArrayBuffer);
                glFlush();
            }
    ...
    bool cgeGenerateVideoWithFilter(const char* outputFilename, const char* inputFilename, const char* filterConfig, float filterIntensity, GLuint texID, CGETextureBlendMode blendMode, float blendIntensity, bool mute, CGETexLoadArg* loadArg)
    {
        ...
        logi("The input audio sample-rate: %d", audioSampleRate);
        ...
        {
            loge("CGEVideoEncoderMP4 - start recording failed!");
            return false;
        }
        ...
        while(1)
        {
            ...
            else if(nextFrameType == FrameType_AudioFrame)
            {
                if(!mute)
                {
                    ...
                    logi("ts: %g, nbSamples: %d, bps: %d, channels: %d, linesize: %d, format: %d", decodeData->timestamp, decodeData->nbSamples, decodeData->bytesPerSample, decodeData->channels, decodeData->linesize, decodeData->format);
