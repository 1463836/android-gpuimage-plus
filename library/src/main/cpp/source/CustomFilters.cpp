...
void *cgeCreateCustomFilter(CustomFilterType type, float intensity, bool useWrapper) {
    ...
    ImageFilterInterface *customFilter = cgeCreateCustomFilterByType(type);

    if (customFilter == nullptr) {
        loge("create Custom filter failed!");;
        return nullptr;
    }
...
jobject
cgeFilterImage_CustomFilters(JNIEnv *env, jobject bmp, CustomFilterType type, float intensity,
                             jboolean hasContext, bool useWrapper) {
    ...
    //similar with cgeNativeLibrary.cpp
    {
        ...
        auto customFilter = (ImageFilterInterface *) cgeCreateCustomFilter(type, intensity,
                                                                           useWrapper);
