...
namespace CGE {
    ...
    public:
        ...
        static void setupTracker(const char *modelfname, const char *trifname,
                                 const char *confname); //使用外部文件初始化
        ...
        */
        ...
        bool updateFace(cv::Mat &faceImage, int framePerDetection = -1, int iteration = 5,
                        double clamp = 3.0, double tolerance = 0.01, bool checkStatus = false);
        ...
        inline const std::vector<Vec2f> &
        getPointList() const { return m_pointList[m_pointListIndex]; }
        ...
        void drawMeshes(cv::Mat &dst, int thickness = 2,
                        const cv::Scalar scalar = cv::Scalar(255, 255, 255, 255)) const;

        void drawFeature(cv::Mat &dst, CGEFaceFeature feature, bool cycle = true, int thickness = 2,
                         const cv::Scalar scalar = cv::Scalar(255, 255, 255, 255)) const;
    ...
    protected:
        ...
        inline std::vector<Vec2f> &getPointListCache() {
            return m_pointList[1 - m_pointListIndex];
        }
