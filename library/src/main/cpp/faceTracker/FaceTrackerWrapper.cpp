...
Java_org_wysaid_nativePort_CGEFaceTracker_nativeSetupTracker(JNIEnv *env, jclass, jstring model,
                                                             jstring
                                                             tri, jstring con) {
    if (model == nullptr || tri == nullptr || con == nullptr) {
        ...
        env->
                ReleaseStringUTFChars(model, modelStr
        );
        env->
                ReleaseStringUTFChars(tri, triStr
        );
        env->
                ReleaseStringUTFChars(con, conStr
        );
    }

    logi("JNI Face Tracker is OK!");
}

JNIEXPORT jlong

JNICALL Java_org_wysaid_nativePort_CGEFaceTracker_nativeCreateFaceTracker(JNIEnv *env, jobject) {
...
JNIEXPORT void JNICALL
Java_org_wysaid_nativePort_CGEFaceTracker_nativeRelease(JNIEnv
                                                        *env, jobject,
                                                        jlong addr
) {
    ...
    delete
            tracker;
    logi("tracker release...");
}

JNIEXPORT jfloatArray
JNICALL Java_org_wysaid_nativePort_CGEFaceTracker_nativeDetectFaceWithSimpleResult(JNIEnv *env,
                                                                                   jobject, jlong
                                                                                   addr,
                                                                                   jobject bmp,
                                                                                   jboolean
    ...
    if (info.format !=
        ANDROID_BITMAP_FORMAT_A_8 && info
                                             .format != ANDROID_BITMAP_FORMAT_RGBA_8888) {
        loge("Bitmap format is not ALPHA_8 !");
        return nullptr;
    }
    ...
    bool suc = tracker->trackImage(row, info.width, info.height, info.stride,
                                   (info.format == ANDROID_BITMAP_FORMAT_A_8 ? 1 : 4), drawFeature);
...
JNIEXPORT jboolean
JNICALL Java_org_wysaid_nativePort_CGEFaceTracker_nativeDetectFaceWithBuffer(JNIEnv *env, jobject,
                                                                             jlong
                                                                             addr,
                                                                             jobject imgBuffer, jint
                                                                             w,
                                                                             jint h, jint
                                                                             channel,
                                                                             jint bytesPerRow,
...
namespace CGE {
    ...
    bool CGEFaceTrackerWrapper::trackContinuousImage(void *buffer, int w, int h, int channel,
                                                     int stride) {
        if (channel == 1) {
    ...
    bool CGEFaceTrackerWrapper::trackImage(void *buffer, int w, int h, int stride, int channel,
                                           bool drawFeature) {
