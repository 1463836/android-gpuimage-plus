...
namespace CGE {
    void
    CGECurveInterface::loadCurves(const float *curveR, size_t nR, const float *curveG, size_t nG,
    ...
    bool CGECurveInterface::loadCurve(std::vector<CurveData> &vec, const float *curve, size_t cnt,
                                      size_t dstChannel, size_t srcChannel, size_t stride) {
    ...
    void
    CGECurveInterface::setPoints(const CurvePoint *r, size_t nr, const CurvePoint *g, size_t ng,
    ...
    bool CGECurveInterface::mergeCurveConst(std::vector<CurveData> &dst,
                                            const std::vector<CurveData> &late,
    ...
    bool CGECurveInterface::mergeCurve(std::vector<float> &dst, std::vector<float> &late,
    ...
    bool CGECurveInterface::mergeCurve(std::vector<CurveData> &dst, std::vector<float> &late,
                                       std::vector<CurveData> &early, unsigned channel) {
    ...
    bool
    CGECurveInterface::_genCurve(float *curve, const CurvePoint *pnts, size_t cnt, unsigned channel,
                                 unsigned stride) {
        if (curve == nullptr)
            return false;
        ...
        for (int i = 0; i != CGE_CURVE_PRECISION; ++i) {
            ...
            const float g = a * pnts[kL].y + b * pnts[kH].y +
                            ((a * a * a - a) * ypp[kL] + (b * b * b - b) * ypp[kH]) * (h * h) /
                            6.0f;
    ...
    bool CGECurveInterface::genCurve(std::vector<float> &vec, const CurvePoint *pnts, size_t cnt) {
        ...
        if (cnt <= 1 || pnts == nullptr) {
            ...
            loge("Invalid Curve Points! Ptr: %p, Count: %d", pnts, (int) cnt);
            return false;
        }
    ...
    bool
    CGECurveInterface::genCurve(std::vector<CurveData> &vec, const CurvePoint *pntsR, size_t cntR,
                                const CurvePoint *pntsG, size_t cntG, const CurvePoint *pntsB,
                                size_t cntB) {
        if (vec.size() != CGE_CURVE_PRECISION) vec.resize(CGE_CURVE_PRECISION);
    ...
    bool
    CGECurveInterface::genCurve(std::vector<CurveData> &vec, const CurvePoint *pnts, size_t cnt,
                                size_t channel) {
        if (vec.size() != CGE_CURVE_PRECISION) vec.resize(CGE_CURVE_PRECISION);
        if (cnt <= 1 || pnts == nullptr || channel > 3) {
            resetCurve(vec, CGE_CURVE_PRECISION);
            loge("Invalid Curve Points! Ptr: %p, Count: %d\n", pnts, (int) cnt);
            return false;
        }
        return _genCurve(&vec[0][0], pnts, cnt, 3, (int) channel);
    }

    void CGECurveInterface::_assignCurveArrays(ProgramObject &program, CGEConstString name,
                                               std::vector<CurveData> &data) {
        program.bind();
        GLint index = program.uniformLocation(name);
        if (index < 0) {
            loge("CGECurveFilter: Failed when assignCurveArray()\n");
            return;
        }
    ...
    void CGECurveInterface::_assignCurveSampler(GLuint &texID, std::vector<CurveData> &data) {
        ...
        if (texID == 0) {
            texID = genTextureWithBuffer(samplerData, int(sz), 1, GL_RGB, dataType, 3, 0,
    ...
    void
    CGECurveTexFilter::setPoints(const CurvePoint *r, size_t nr, const CurvePoint *g, size_t ng,
                                 const CurvePoint *b, size_t nb) {
    ...
    void
    CGECurveTexFilter::loadCurves(const float *curveR, size_t nR, const float *curveG, size_t nG,
                                  const float *curveB, size_t nB) {
    ...
    void CGECurveFilter::setPoints(const CurvePoint *r, size_t nr, const CurvePoint *g, size_t ng,
                                   const CurvePoint *b, size_t nb) {
        CGECurveInterface::setPoints(r, nr, g, ng, b, nb);
        flush();
    }
    ...
    void CGECurveFilter::loadCurves(const float *curveR, size_t nR, const float *curveG, size_t nG,
                                    const float *curveB, size_t nB) {
        CGECurveInterface::loadCurves(curveR, nR, curveG, nG, curveB, nB);
        flush();
    }
    ...
    void
    CGEMoreCurveFilter::pushPoints(const CurvePoint *r, size_t nr, const CurvePoint *g, size_t ng,
                                   const CurvePoint *b, size_t nb) {
        pushPointsR(r, nr);
        pushPointsG(g, ng);
        pushPointsB(b, nb);
    ...
    void
    CGEMoreCurveFilter::pushCurves(const float *curveR, size_t nR, const float *curveG, size_t nG,
                                   const float *curveB, size_t nB) {
