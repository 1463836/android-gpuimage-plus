...
namespace CGE {
    ImageHandlerInterface::ImageHandlerInterface() : srcTexture(0), fbo(0),
                                                     vertexArrayBuffer(0) {
...
//        logi("Image Handler Init With RawBufferData %d x %d, %d channel\n", w, h, channel);
        ...
        glDeleteTextures(1, &srcTexture); //Delete last texture to avoid reinit error.
        revertEnabled = enableReversion;//false
        if (revertEnabled) {
            srcTexture = genTextureWithBuffer(bufferdata, dstWidth, dstHeight, channelFmt, dataFmt, channel);
