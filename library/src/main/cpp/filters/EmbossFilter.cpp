                                       ...
                                       (
                                               ...
                                               void main() {
                                                   vec4 src = texture2D(inputImageTexture,
                                                                        textureCoordinate);
                                                   vec3 tmp = texture2D(inputImageTexture,
                                                                        textureCoordinate +
                                                                        samplerSteps * stride *
                                                                        norm).rgb - src.rgb + 0.5;
                                                   float f = (tmp.r + tmp.g + tmp.b) / 3.0;
                                                   gl_FragColor = vec4(
                                                           mix(src.rgb, vec3(f, f, f), intensity),
                                                           src.a);
                                               }
                                       );

namespace CGE {
    ConstString CGEEmbossFilter::paramIntensity = "intensity";
    ConstString CGEEmbossFilter::paramStride = "stride";
    ConstString CGEEmbossFilter::paramNorm = "norm";
