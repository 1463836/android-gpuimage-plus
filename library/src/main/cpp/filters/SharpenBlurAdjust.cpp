...
const static char *const s_fshFastBlur = CGE_SHADER_STRING_PRECISION_H
                                         (
                                                 ...
                                                 float random(vec2 seed) {
                                                     return fract(
                                                             sin(dot(seed, vec2(12.9898, 78.233))) *
                                                             43758.5453);
                                                 }
                                                 ...
                                                 void main() {
                                                     ...
                                                     for (int i = -samplerRadius;
                                                          i <= samplerRadius; ++i) {
                                                         float percent = (float(i) + offset) /
                                                                         float(samplerRadius);
                                                         float weight = 1.0 - abs(percent);
                                                         vec2 coord = textureCoordinate +
                                                                      samplerSteps * percent *
                                                                      blurNorm *
                                                                      float(blurSamplerScale);
                                                         resultColor += texture2D(inputImageTexture,
                                                                                  coord) * weight;
                                                         blurPixels += weight;
                                                     }
...
const static char *const s_fshFixedRadiusBlur = CGE_SHADER_STRING_PRECISION_H
                                                (
                                                        ...
                                                        float random(vec2 seed) {
                                                            return fract(sin(dot(seed, vec2(12.9898,
                                                                                            78.233))) *
                                                                         43758.5453);
                                                        }
...
static ConstString s_fshMixTex = CGE_SHADER_STRING_PRECISION_H
                                    (
                                            varying vec2 textureCoordinate;
                                            uniform sampler2D inputImageTexture;
                                            ...
                                            void main() {
                                                vec4 src = texture2D(inputImageTexture,
                                                                     textureCoordinate);
                                                vec4 tex = texture2D(blurredImageTexture,
                                                                     textureCoordinate);
                                                gl_FragColor = mix(tex, src, intensity);
                                            }
                                    );

static ConstString s_fshSharpenBlur_Simple = CGE_SHADER_STRING_PRECISION_H
                                                (
                                                        varying vec2 textureCoordinate;
                                                        uniform sampler2D inputImageTexture;
                                                        uniform vec2 samplerSteps;
                                                        ...
                                                        void main() {
                                                            ...
                                                            vec4 tmp = src +
                                                                       ...
                                                                       texture2D(inputImageTexture,
                                                                                 ...
                                                                                 vec2(0.0,
                                                                                      samplerSteps.y));
                                                            gl_FragColor = mix(tmp / 5.0, src,
                                                                               intensity);
                                                        }
                                                );

static ConstString s_fshSharpenBlur_SimpleBetter = CGE_SHADER_STRING_PRECISION_H
                                                      (
                                                              varying vec2 textureCoordinate;
                                                              uniform sampler2D inputImageTexture;
                                                              uniform vec2 samplerSteps;
                                                              ...
                                                              void main() {
                                                                  ...
                                                                  tmp += texture2D(
                                                                          ...
                                                                          vec2(samplerSteps.x,
                                                                               -samplerSteps.y));
                                                                  gl_FragColor = mix(tmp / 9.0, src,
                                                                                     intensity);
                                                              }
                                                      );

namespace CGE {
    ConstString CGESharpenBlurFilter::paramIntensity = "intensity";
    ConstString CGESharpenBlurFilter::paramSamplerScale = "blurSamplerScale";
    ConstString CGESharpenBlurFilter::paramSamplerStepName = "samplerSteps";
    ConstString CGESharpenBlurFilter::paramBlurNormalName = "blurNorm";
    ConstString CGESharpenBlurFilter::paramSamplerRadius = "samplerRadius";
    ...
    void CGESharpenBlurFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
    ...
    void CGEBlurFastFilter::render2Texture(ImageHandlerInterface *handler, GLuint dstTexture,
                                           GLuint vertexBufferID) {
        const Sizei &sz = handler->getOutputFBOSize();
    ...
    ConstString CGESharpenBlurFastFilter::paramIntensity = "intensity";
    ConstString CGESharpenBlurFastFilter::paramBlurredTexName = "blurredImageTexture";

    bool CGESharpenBlurFastFilter::init() {
        if (m_blurProc.initWithoutFixedRadius() &&
            initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshMixTex)) {
    ...
    void
    CGESharpenBlurFastFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
                                             GLuint vertexBufferID) {
        handler->setAsTarget();
        m_program.bind();
        const Sizei &sz = handler->getOutputFBOSize();
        if (m_texture == 0) {
            m_texture = genTextureWithBuffer(nullptr, sz.width, sz.height, GL_RGBA,
                                             GL_UNSIGNED_BYTE);
        ...
        ImageFilterInterface::render2Texture(handler, srcTexture, vertexBufferID);
    }
    ...
    ConstString CGESharpenBlurSimpleFilter::paramIntensity = "intensity";
    ConstString CGESharpenBlurSimpleFilter::paramSamplerScale = "blurSamplerScale";
    ConstString CGESharpenBlurSimpleFilter::paramSamplerStepName = "samplerSteps";
