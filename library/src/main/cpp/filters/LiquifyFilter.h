...
namespace CGE {
    ...
    class LiquifyFilter : public ImageFilterInterface {
    public:
        LiquifyFilter();
        ...
        virtual void forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
                                       float intensity); //向前变形工具
        virtual void
        pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity,
                           float angle = 3.14159f /
                                         2.0f);//(左)推动工具：将路径上的网格向路径的angle方向（路径的左侧）推动。 末尾参数angle默认为左侧，可设置为任意值
        virtual void
        restoreMeshWithPoint(Vec2f pnt, float w, float h, float radius, float intensity);//重建工具
        virtual void
        bloatMeshWithPoint(Vec2f pnt, float w, float h, float radius, float intensity);//膨胀工具
        virtual void
        wrinkleMeshWithPoint(Vec2f pnt, float w, float h, float radius, float intensity);//褶皱工具
    ...
    protected:
        static ConstString paramTextureVertexName;
    ...
    class CGELiquidationNicerFilter : public LiquifyFilter {
    public:
        void forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
                               float intensity);//see in 'CGELiquifyFilter'
        virtual void
        pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity,
                           float angle = 3.14159f / 2.0f);//see in 'CGELiquifyFilter'
    };
