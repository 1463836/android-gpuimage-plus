...
static ConstString s_vshDeform = CGE_SHADER_STRING
(
...
static ConstString s_fshDeform = CGE_SHADER_STRING_PRECISION_M
                                    (
                                            varying vec2 textureCoordinate;
...
static ConstString s_fshMesh = CGE_SHADER_STRING_PRECISION_L
                                  (
                                          varying vec2 textureCoordinate;
                                          uniform sampler2D inputImageTexture;
                                          void main() {
                                              gl_FragColor.rgb = 1.0 - texture2D(inputImageTexture,
                                                                                 textureCoordinate).rgb;
                                          }
                                  );
...
namespace CGE {
    ...
    ConstString LiquifyFilter::paramTextureVertexName = "vTexture";

    LiquifyFilter::LiquifyFilter() : m_meshVBO(0), m_meshIndexVBO(0), m_textureVBO(0),
                                     m_currentMeshIndex(0), m_doingRestore(false) {
#ifdef CGE_DEFORM_SHOW_MESH
    ...
    bool LiquifyFilter::initWithMesh(float width, float height, float stride) {
        return initWithMesh(width / height, stride / CGE_MAX(width, height));
    }

    bool LiquifyFilter::initWithMesh(float ratio, float stride) {
        ...
        CGE_LOG_CODE(
                if (ratio < 0.0f) {
                    loge("DeformProcessor::initWithMesh Ratio must > 0!\n");
                    return false;
                }
        );
    ...
    void LiquifyFilter::restoreMesh() {
    ...
    void LiquifyFilter::restoreMeshWithIntensity(float intensity) {
        if (m_mesh.size() != m_meshSize.width * m_meshSize.height || m_mesh.empty()) {
            loge("Invalid Mesh!\n");
            return;
        }
        ...
        if (!m_doingRestore && !pushMesh()) {
            loge("DeformProcessor::restoreMeshWithIntensity failed!\n");
            return;
        }
    ...
    bool LiquifyFilter::initBuffers() {
        glDeleteBuffers(1, &m_meshVBO);
        glGenBuffers(1, &m_meshVBO);
        glBindBuffer(GL_ARRAY_BUFFER, m_meshVBO);
        glBufferData(GL_ARRAY_BUFFER, m_mesh.size() * sizeof(m_mesh[0]), m_mesh.data(),
                     GL_STREAM_DRAW);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshIndexes.size() * sizeof(meshIndexes[0]),
                     meshIndexes.data(), GL_STATIC_DRAW);
    ...
    bool LiquifyFilter::updateBuffers() {
        if (m_meshVBO != 0) {
            glBindBuffer(GL_ARRAY_BUFFER, m_meshVBO);
            glBufferData(GL_ARRAY_BUFFER, sizeof(m_mesh[0]) * m_mesh.size(), m_mesh.data(),
                         GL_STREAM_DRAW);
            glBindBuffer(GL_ARRAY_BUFFER, 0);
            return true;
        }
    ...
    void LiquifyFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
                                       GLuint vertexBufferID) {
        if (m_meshVBO == 0 || m_mesh.empty()) {
            loge("DeformProcessor::render2Texture - Invalid Mesh!\n");
    ...
    void LiquifyFilter::forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
                                          float intensity) {
        ...
        logi("Canvas Size: %g, %g\nBoundBox: left:%g, top: %g, right: %g, bottom: %g\n", w,
             h, loopStartX, loopStartY, loopEndX, loopEndY);
        ...
        CGE_LOG_CODE(
                logi("##########Deform mesh take time: %gs #####\n",
                             float(clock() - t) / CLOCKS_PER_SEC);
        );
    }

    void
    LiquifyFilter::pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
    ...
    void LiquifyFilter::restoreMeshWithPoint(Vec2f pnt, float w, float h, float radius,
                                             float intensity) {
    ...
    void LiquifyFilter::bloatMeshWithPoint(Vec2f pnt, float w, float h, float radius,
                                           float intensity) {
    ...
    void LiquifyFilter::wrinkleMeshWithPoint(Vec2f pnt, float w, float h, float radius,
                                             float intensity) {
    ...
    void LiquifyFilter::setUndoSteps(unsigned n) {
        m_undoSteps = n;
        if (n == 0)
            m_vecMeshes.clear();
    ...
    void CGELiquidationNicerFilter::forwardDeformMesh(Vec2f start, Vec2f end, float w, float h,
        ...
        //计算直线方程
        {
            float a = start[1] - end[1], b = start[0] - end[0], c =
    ...
    void CGELiquidationNicerFilter::pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h,
                                                       float radius, float intensity, float angle) {
    ...
    void LiquifyFilter::showMesh(bool bShow) {
#ifdef CGE_DEFORM_SHOW_MESH
        m_bShowMesh = bShow;
#endif
    }
