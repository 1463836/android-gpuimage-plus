...
void getScale(GLfloat *colorScaleLow, GLfloat *colorScaleRange, GLfloat *sat, GLfloat dark,
              GLfloat light, GLint w, GLint h) {
...
void getHalfToneLowFac(GLfloat *low, GLfloat *fac, GLint w, GLint h) {
    ...
    for (int i = 0; i != total; ++i) {
        ++lum[(unsigned char) ((data[0] * int(256 * 0.299) + data[1] * int(256 * 0.587) +
                                data[2] * int(256 * 0.114)) >> 8)];
        data += 4;
    }
...
static ConstString s_fshLomo = CGE_SHADER_STRING_PRECISION_M
                                  (
                                          ...
                                          void main() {
                                              ...
                                              float percent = clamp((d - vignette.x) / vignette.y,
                                                                    0.0, 1.0);
                                              float alpha = 1.0 - percent * percent;

                                              float lum = dot(src.rgb, vec3(0.299, 0.587, 0.114));
                                              vec3 dst = lum * (1.0 - saturation) +
                                                         saturation * src.rgb;
                                              dst = (dst - colorScale.x) / colorScale.y * alpha;

                                              gl_FragColor = vec4(mix(src.rgb, dst, intensity),
                                                                  src.a);
                                          }
                                  );

static ConstString s_fshLomoLinear = CGE_SHADER_STRING_PRECISION_M
                                        (
                                                varying vec2 textureCoordinate;
                                                uniform sampler2D inputImageTexture;
                                                ...
                                                void main() {
                                                    ...
                                                    float d = distance(textureCoordinate,
                                                                       vignetteCenter);
                                                    float percent = clamp(
                                                            (d - vignette.x) / vignette.y, 0.0,
                                                            1.0);
                                                    float alpha = 1.0 - percent;

                                                    float lum = dot(src.rgb,
                                                                    vec3(0.299, 0.587, 0.114));
                                                    vec3 dst = lum * (1.0 - saturation) +
                                                               saturation * src.rgb;
                                                    dst = (dst - colorScale.x) / colorScale.y *
                                                          alpha;

                                                    gl_FragColor = vec4(
                                                            mix(src.rgb, dst, intensity), src.a);
                                                }
                                        );

//////////////////////////////////////////////////////////////////////////

static ConstString s_fshLomoWithCurve = CGE_SHADER_STRING_PRECISION_M
                                           (
                                                   varying vec2 textureCoordinate;
                                                   uniform sampler2D inputImageTexture;
                                                   ...
                                                   void main() {
                                                       ...
                                                       gl_FragColor = vec4(curveArray[int(
                                                               dst.r * curvePrecision)].r,
                                                                           curveArray[int(dst.g *
                                                                                          curvePrecision)].g,
                                                                           curveArray[int(dst.b *
                                                                                          curvePrecision)].b,
                                                                           src.a);
                                                   }
                                           );

static ConstString s_fshLomoWithCurveLinear = CGE_SHADER_STRING_PRECISION_M
                                                 (
                                                         varying vec2 textureCoordinate;
                                                         uniform sampler2D inputImageTexture;
                                                         uniform vec3 curveArray[256]; // The array size is defined by "CURVE_PRECISION".
...
static ConstString s_fshLomoWithCurveTex = CGE_SHADER_STRING_PRECISION_M
                                              (
                                                      varying vec2 textureCoordinate;
                                                      uniform sampler2D inputImageTexture;
...
static ConstString s_fshLomoWithCurveTexLinear = CGE_SHADER_STRING_PRECISION_M
                                                    (
                                                            varying vec2 textureCoordinate;
                                                            uniform sampler2D inputImageTexture;
                                                            uniform sampler2D curveTexture;
                                                            ...
                                                            const vec2 vignetteCenter = vec2(0.5,
                                                                                             0.5);
...
namespace CGE {
    ConstString CGELomoFilter::paramColorScaleName = "colorScale";
    ConstString CGELomoFilter::paramSaturationName = "saturation";
    ConstString CGELomoFilter::paramVignetteName = "vignette";
    ConstString CGELomoFilter::paramAspectRatio = "aspectRatio";
    ConstString CGELomoFilter::paramIntensityName = "intensity";
    ...
    void CGELomoFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
        ...
        //More initialization...
        {
            ...
            if (s_colorScaleStatus > 0 && m_scaleDark >= 0.0f && m_scaleLight > 0.0f) {
                ...
                CGE_LOG_CODE(clock_t
                                     t = clock());
                getScale(colorScale, colorScale + 1, &sat, m_scaleDark, m_scaleLight, sz.width,
                         sz.height);
                logi("getscale用时%gs\n", (double) (clock() - t) / CLOCKS_PER_SEC);
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                       handler->getTargetTextureID(), 0);
            }
    ...
    ConstString CGELomoWithCurveFilter::paramColorScaleName = "colorScale";
    ConstString CGELomoWithCurveFilter::paramSaturationName = "saturation";
    ConstString CGELomoWithCurveFilter::paramVignetteName = "vignette";
    ConstString CGELomoWithCurveFilter::paramAspectRatio = "aspectRatio";
    ...
    void
    CGELomoWithCurveFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
                                           GLuint vertexBufferID) {
        handler->setAsTarget();
        m_program.bind();
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, 0);
    ...
    void CGELomoWithCurveTexFilter::flush() {
        ...
        if (m_curveTexture == 0) {
            m_curveTexture = genTextureWithBuffer(samplerData, int(sz), 1, GL_RGB, dataType, 3,
                                                  0, GL_NEAREST, GL_CLAMP_TO_EDGE);
        } else {
...
static ConstString s_fshColorScale = CGE_SHADER_STRING_PRECISION_M
                                        (
                                                varying vec2 textureCoordinate;
                                                uniform sampler2D inputImageTexture;
                                                ...
                                                void main() {
                                                    ...
                                                    float lum = dot(src.rgb,
                                                                    vec3(0.299, 0.587, 0.114));
                                                    src.rgb = lum * (1.0 - saturation) +
                                                              saturation * src.rgb;
                                                    src.rgb =
                                                            (src.rgb - colorScale.x) / colorScale.y;
                                                    gl_FragColor = src;
                                                }
                                        );

namespace CGE {
    ConstString CGEColorScaleFilter::paramColorScaleName = "colorScale";
    ConstString CGEColorScaleFilter::paramSaturationName = "saturation";
    ...
    void CGEColorScaleFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
                                             GLuint vertexBufferID) {
        handler->setAsTarget();
        m_program.bind();
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, 0);
...
namespace CGE {
    ConstString CGEColorMulFilter::paramMulName = "cmul";
    ...
    bool CGEColorMulFilter::initWithMode(MulMode mode) {
        ...
        switch (mode) {
            ...
            case mulMAT:
                ...
                break;
            default:
                loge("CGEColorMulFilter - Invalid mode name: %d\n", mode);
                return false;
        }
    ...
    void CGEColorMulFilter::setMAT(float *mat) {
        m_program.bind();
        GLint index = glGetUniformLocation(m_program.getProgramID(), paramMulName);
        if (index < 0) {
            loge("CGEColorMulFilter::setMAT - uniform name %s doesnot exist!\n",
                 paramMulName);
            return;
        }
