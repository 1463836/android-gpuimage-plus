...
#define LOG_ERROR_PARAM(arg) loge("Invalid Parameters: %s\n", arg);

namespace CGE {
    extern bool g_isFastFilterImpossible;

    void tableParserHelper(std::vector<CGECurveTexFilter::CurvePoint> &vecPnts, const char *pstr,
                           int n) {
        const char *p = pstr;
        int a, b;
...
tableParserHelper(vec, pstr + n, i - n); \
    ...
    CGEDataParsingEngine::curveParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace std;
        ...
        if (proc == nullptr) {
            logi(
                    "curveParser - Curve With Texture is used!(Not error, everything is ok)\n");
            proc = createMoreCurveTexFilter();

            if (proc == nullptr) {
                loge("CGEDataParsingEngine::curveParser Create Curve filter Failed!\n");
                return nullptr;
            }
        }
            ...
            switch (pstr[i]) {
                ...
                case 'r':
                    if (toupper(pstr[i + 1]) == 'G' && toupper(pstr[i + 2]) == 'B') {
                        ...
                        if (vecRGB.size() < 2) {
                            loge("Not enough RGB curve points: %s\n", pstr);
                        } else {
                        ...
                        int n = i;
                        for (char c = toupper(pstr[i]);
                             c != '\0' && c != 'R' && c != 'G' && c != 'B' && c != '@'; c = toupper(
                                pstr[i]))
                            ++i;
                        tableParserHelper(vecR, pstr + n, i - n);
                        if (vecR.size() < 2) {
                            loge("Not enough R curve points: %s\n", pstr);
                        } else {
                ...
                case 'g':
                    ...
                    if (vecG.size() < 2) {
                        loge("Not enough G curve points: %s\n", pstr);
                    } else {
                ...
                case 'b':
                    ...
                    if (vecB.size() < 2) {
                        loge("Not enough B curve points: %s\n", pstr);
                    } else {
    ...
    ImageFilterInterface *CGEDataParsingEngine::lomoWithCurveParser(const char *pstr,
        ...
        if (sscanf(pstr, "%f%*c%f%*c%f%*c%f%*c%f%*c%d",
                   &vignetteStart, &vignetteEnd, &colorScaleLow, &colorScaleRange, &saturation,
                   &isLinear) < 5) {
            return nullptr;
        }

        CGELomoWithCurveFilter *proc = g_isFastFilterImpossible ? nullptr : (isLinear
                                                                             ? new CGELomoWithCurveLinearFilter
                                                                             : new CGELomoWithCurveFilter);

        if (proc == nullptr || !proc->init()) {
            delete proc;
            proc = isLinear ? new CGELomoWithCurveTexLinearFilter : new CGELomoWithCurveTexFilter;
            if (!proc->init()) {
                loge("CGEDataParsingEngine::lomoWithCurveParser Create filter Failed!\n");
                delete proc;
                return nullptr;
            }
            logi(
                    "lomoWithCurveParser - Curve With Texture is used!(Not error, everything is ok)\n");
        }
        ...
        if (vecRGB.empty() && vecR.empty() && vecG.empty() && vecB.empty()) {
            loge("lomoParser - Warning: Empty Curve!!\n");
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::lomoParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace std;
...
do{\
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::adjustParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        while (*pstr != '\0' && (*pstr == ' ' || *pstr == '\t')) ++pstr;
        ImageFilterInterface *proc = nullptr;
        ...
        *pBuffer = '\0';
        ...
        if (strcmp(buffer, "brightness") == 0) {
            ...
            if (sscanf(pstr, "%f", &intensity) != 1)
                return nullptr;
            CGEBrightnessFastFilter *bfp = g_isFastFilterImpossible ? nullptr
                                                                    : createBrightnessFastFilter();
            ...
            if (bfp != nullptr) {
                ...
                loge(
                        "CGEDataParsingEngine::adjustParser Create Brightness filter Failed\n");
                return nullptr;
            }
            ...
            float temp, tint;
            if (sscanf(pstr, "%f%*c%f", &temp, &tint) != 2) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
            CGEWhiteBalanceFastFilter *wfp = g_isFastFilterImpossible ? nullptr
                                                                      : createWhiteBalanceFastFilter();
            ...
            if (wfp != nullptr) {
                ...
                loge(
                        "CGEDataParsingEngine::adjustParser Create WhiteBalance filter Failed\n");
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%f%*c%f%*c%f%*c%f%*c%f%*c%f",
                ...
                loge("adjust hsv - Invalid Parameters: %s\n", pstr);
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%f%*c%f", &shadow, &highlight) != 2) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
            CGEShadowHighlightFastFilter *shfp = g_isFastFilterImpossible ? nullptr
                                                                          : createShadowHighlightFastFilter();
            CGEShadowHighlightFilter *shp = (shfp == nullptr) ? createShadowHighlightFilter()
                                                              : nullptr;
            if (shfp != nullptr) {
                ...
                loge(
                        "CGEDataParsingEngine::adjustParser Create ShadowHighlight filter Failed\n");
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%f%*c%f%*c%f", &h, &s, &l) != 3) {
                loge("adjust hsl - Invalid Parameters: %s\n", pstr);
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%f%*c%f%*c%f", &dark, &light, &gamma) != 3) {
                loge("adjust color level - Invalid Parameters: %s\n", pstr);
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%f%*c%f%*c%f", &red, &green, &blue) != 3) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
            ...
            if (filter != nullptr) {
                ...
                loge(
                        "CGEDataParsingEngine::adjustParser Create ColorBalance filter Failed\n");
                return nullptr;
            }
        } else if (strcmp(buffer, "lut") == 0) {
            char lutName[128];
            if (sscanf(pstr, "%127s", lutName) != 1) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
            ...
            if (filter != nullptr && tex != 0) {
                ...
                loge("CGEDataParsingEngine::adjustParser Create Lookup filter Failed\n");
            }
        } else { LOG_ERROR_PARAM(pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::blendParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        char modeName[32], textureName[128];
        int intensity;

        if (sscanf(pstr, "%31s%127s%d", modeName, textureName, &intensity) != 3) {
            loge("blendParser - Invalid Param: %s\n", pstr);
            return nullptr;
        }
        ...
        if (sscanf(textureName, "[%d%*c%d%*c%d]", &texID, &texWidth, &texHeight) != 3 ||
            texID == 0) {
        ...
        (
                else
                {
                    if (!glIsTexture(texID)) {
                        loge(
                                "Warn: special usage with texture id, but the texture id is not valid now.");
                    }
                }
        )

        if (texID == 0) {
            loge("blend - %s : loadResources failed: %s\n", modeName, textureName);
            delete proc;
            return nullptr;
        }
    ...
    ImageFilterInterface *CGEDataParsingEngine::vignetteBlendParser(const char *pstr,
                                                                    CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        ...
        if (sscanf(pstr, "%" BUFFER_LEN_STR "s%f%f%f%f%f%f%f%f%f%d", blendMethod, color, color + 1,
                   color + 2, color + 3, &intensity, &low, &range, &centerX, &centerY, &kind) <
            10) {
            loge("vignetteBlendParser - Invalid parameters: %s\n", pstr);
            return nullptr;
        }
        ...
        switch (kind) {
            ...
            case 3:
                ...
                break;
            default:
                loge("vignetteBlendParser - Invalid vignette kind %d", kind);
                return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::colorScaleParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        float low, range, sat;
        if (sscanf(pstr, "%f%*c%f%*c%f", &low, &range, &sat) != 3) {
            loge("colorScaleParser - Invalid Parameters: %s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::pixblendParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        char blendMethod[BUFFER_LEN];
        float color[4], intensity;

        if (sscanf(pstr, "%" BUFFER_LEN_STR "s%f%f%f%f%f", blendMethod, color, color + 1, color + 2,
                   color + 3, &intensity) != 6) {
            loge("pixblendParser - Invalid parameters: %s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::krblendParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        char modeName[32], textureName[128];
        int intensity;
        if (sscanf(pstr, "%31s%127s%d", modeName, textureName, &intensity) != 3) {
            loge("krblendParser - Invalid Param: %s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::vignetteParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        float low, range, centerX, centerY;
        int n = sscanf(pstr, "%f%*c%f%*c%f%*c%f", &low, &range, &centerX, &centerY);
        if (n < 2) {
            loge("vignetteParser - Invalid Param: %s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::selfblendParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        char modeName[32];
        int intensity;
        if (sscanf(pstr, "%31s%d", modeName, &intensity) != 2) {
            loge("selfblendParser - Invalid Param: %s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::colorMulParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        using namespace CGE;
        char funcName[32] = "";
        if (sscanf(pstr, "%31s", funcName) != 1) {
            loge("colorMulParser - Invalid Param: %s", pstr);
            return nullptr;
        }
        ...
        if (strncmp(funcName, "flt", 3) == 0) {
            ...
            if (sscanf(pstr, "%*s%f", &value) != 1) {
                loge("colorMulParser - flt - Invalid Param:%s\n", pstr);
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%*s%f%*c%f%*c%f", &r, &g, &b) != 3) {
                loge("colorMulParser - vec - Invalid Param:%s\n", pstr);
                return nullptr;
            }
            proc = new CGEColorMulFilter;
            ...
            if (sscanf(pstr, "%*s%f%*c%f%*c%f%*c%f%*c%f%*c%f%*c%f%*c%f%*c%f",
                       ...
                       mat + 8) != 9) {
                loge("colorMulParser - mat - Invalid Param:%s\n", pstr);
                return nullptr;
            }
            proc = new CGEColorMulFilter;
            ...
            loge("colorMulParser - Invalid Param:%s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *CGEDataParsingEngine::selectiveColorParser(const char *pstr,
                                                                     CGEMutipleEffectFilter *fatherFilter) {
        CGESelectiveColorFilter *proc = createSelectiveColorFilter();
        if (proc == nullptr) {
            loge("selectiveColorParser - init processor failed!\n");
            return nullptr;
        }
        ...
        while (pstr != nullptr && *pstr != '\0' && *pstr != '@') {
            ...
            if (*pstr == '\0' || *pstr == '@')
                break;
            if (sscanf(pstr, "%31[^( \t\n]%*[^-0-9.]%f%*c%f%*c%f%*c%f", funcName, &cyan, &magenta,
                       &yellow, &key) != 5) {
                loge("selectiveColorParser - Invalid Param %s!\n", pstr);
                break;
            }

            while (*pstr != '\0' && *pstr++ != ')');

            if (fabsf(cyan) > 1.0f || fabsf(magenta) > 1.0f || fabsf(yellow) > 1.0f ||
                fabsf(key) > 1.0f) {
                cyan /= 100.0f;
                magenta /= 100.0f;
                yellow /= 100.0f;
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::blendTileParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        char modeName[32], textureName[128];
        int intensity;

        if (sscanf(pstr, "%31s%127s%d", modeName, textureName, &intensity) != 3) {
            loge("blendTileParser - Invalid Param: %s\n", pstr);
            return nullptr;
        }
    ...
    ImageFilterInterface *CGEDataParsingEngine::advancedStyleParser(const char *pstr,
                                                                    CGEMutipleEffectFilter *fatherFilter) {
        while (*pstr != '\0' && (*pstr == ' ' || *pstr == '\t')) ++pstr;
        ImageFilterInterface *proc = nullptr;
        ...
        *pBuffer = '\0';
        ...
        if (strcmp(buffer, "crosshatch") == 0) {
            ...
            if (sscanf(pstr, "%f%*c%f%*c%f%*c%f%*c%f", &dis, &slope, &r, &g, &b) !=
                5) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%127s%*c%d%*c%d%*c%d%*c%d", filename, &mappingWidth, &mappingHeight,
                       &unitWidth, &unitHeight) != 5) { LOG_ERROR_PARAM(buffer);
                return nullptr;
            }
            ...
            if (texID == 0) {
                loge("Load texture %s failed!\n", filename);
                return nullptr;
            }

            CGEColorMappingFilter *filter = CGEColorMappingFilter::createWithMode(
                    CGEColorMappingFilter::MAPINGMODE_DEFAULT);
            ...
            for (int i = 0; i != unitHeight; ++i) {
                for (int j = 0; j != unitWidth; ++j) {
                    CGEColorMappingFilter::MappingArea ma = {
                            Vec4f(j / (float) unitWidth, i / (float) unitHeight, 1.0f / unitWidth,
                                  1.0f / unitHeight),
                            weight
                    };
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::beautifyParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        while (*pstr != '\0' && (*pstr == ' ' || *pstr == '\t')) ++pstr;
        ImageFilterInterface *proc = nullptr;
        ...
        *pBuffer = '\0';
        ...
        if (strcmp(buffer, "bilateral") == 0) {
            ...
            int repeatTimes = 1;
            if (sscanf(pstr, "%f%*c%f%*c%d", &blurScale, &disFactor, &repeatTimes) <
                2) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
            ...
            if (sscanf(pstr, "%f%*c%f%*c%f", &intensity, &width, &height) < 1) { LOG_ERROR_PARAM(
                        pstr);
                return nullptr;
            }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::blurParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        while (*pstr != '\0' && (*pstr == ' ' || *pstr == '\t')) ++pstr;
        ImageFilterInterface *proc = nullptr;
        ...
        *pBuffer = '\0';
        ...
        if (strcmp(buffer, "lerp") == 0) {
            ...
            int argNum;
            if ((argNum = sscanf(pstr, "%f%*c%f", &intensity, &base)) < 1) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
    ...
    ImageFilterInterface *
    CGEDataParsingEngine::dynamicParser(const char *pstr, CGEMutipleEffectFilter *fatherFilter) {
        while (*pstr != '\0' && (*pstr == ' ' || *pstr == '\t')) ++pstr;
        ImageFilterInterface *proc = nullptr;
        ...
        *pBuffer = '\0';
        ...
        if (strcmp(buffer, "wave") == 0) {
            ...
            int argNum = sscanf(pstr, "%f%*c%f%*c%f%*c%f", &motion, &angle, &strength,
                                &motionSpeed);

            if (!(argNum == 3 || argNum == 4 || (argNum == 1 && motion > 0))) { LOG_ERROR_PARAM(
                        pstr);
                return nullptr;
            }
            ...
            if (filter != nullptr) {
                ...
                switch (argNum) {
                    ...
                    default:
                        loge(
                                "Error which should never happen, but just happened... biu biu...\n");
                        delete filter;
                        break;
                }
            }
            ...
            if (sscanf(pstr, "%d%*c%d", &totalFrames, &framesDelay) != 2) { LOG_ERROR_PARAM(pstr);
                return nullptr;
            }
