...
//const int GAUSSIAN_SAMPLES = 9;
                                            ...
                                            float random(vec2 seed) {
                                                return fract(sin(dot(seed, vec2(12.9898, 78.233))) *
                                                             43758.5453);
                                            }

                                            void main() {
                                                vec4 centralColor = texture2D(inputImageTexture,
                                                                              textureCoordinate);
                                                ...
                                                for (int i = 0; i < samplerRadius; ++i) {
                                                    ...
                                                    float blurfactor = blurFactors[samplerRadius -
                                                                                   i];

                                                    {
                                                        vec4 sampleColor1 = texture2D(
                                                                inputImageTexture,
                                                                textureCoordinate + dis);
                                                        float distanceFromCentralColor1 = min(
                                                                distance(centralColor,
                                                                         sampleColor1) *
                                                                distanceNormalizationFactor, 1.0);
                                                        float gaussianWeight1 = blurfactor * (1.0 -
                                                                                              distanceFromCentralColor1);
                                                    ...
                                                    {
                                                        vec4 sampleColor2 = texture2D(
                                                                inputImageTexture,
                                                                textureCoordinate - dis);
                                                        float distanceFromCentralColor2 = min(
                                                                distance(centralColor,
                                                                         sampleColor2) *
                                                                distanceNormalizationFactor, 1.0);
                                                        float gaussianWeight2 = blurfactor * (1.0 -
                                                                                              distanceFromCentralColor2);
...
ConstString s_fshBilateralBlur2 = CGE_SHADER_STRING_PRECISION_H
                                     (
                                             varying vec2 textureCoordinate;
                                             uniform sampler2D inputImageTexture;
                                             ...
                                             void main() {
                                                 vec4 centralColor = texture2D(inputImageTexture,
                                                                               textureCoordinate);
                                                 float lum = dot(centralColor.rgb,
                                                                 vec3(0.299, 0.587, 0.114));
                                                 float factor = (1.0 + arg) / (arg + lum) *
                                                 ...
                                                 for (int i = 1; i <= samplerRadius; ++i) {
                                                     vec2 dis = (float(i) + offset) * stepScale;
                                                     float percent = 1.0 - (float(i) + offset) /
                                                                           float(samplerRadius);

                                                     {
                                                         ...
                                                         float distanceFromCentralColor1 = min(
                                                                 distance(centralColor,
                                                                          sampleColor1) * factor,
                                                                 1.0);
                                                         float gaussianWeight1 = percent * (1.0 -
                                                                                            distanceFromCentralColor1);
                                                         gaussianWeightTotal += gaussianWeight1;
                                                         sum += sampleColor1 * gaussianWeight1;
                                                     }
...

                                                     {
                                                         ...
                                                         float distanceFromCentralColor2 = min(
                                                                 distance(centralColor,
                                                                          sampleColor2) * factor,
                                                                 1.0);
                                                         float gaussianWeight2 = percent * (1.0 -
                                                                                            distanceFromCentralColor2);
                                                         gaussianWeightTotal += gaussianWeight2;
                                                         sum += sampleColor2 * gaussianWeight2;
                                                     }
...
namespace CGE {
    ConstString CGEBilateralBlurFilter::paramDistanceFactorName = "distanceNormalizationFactor";
    ConstString CGEBilateralBlurFilter::paramBlurSamplerScaleName = "blurSamplerScale";
    ConstString CGEBilateralBlurFilter::paramBlurFactorsName = "blurFactors";
    ...
    bool CGEBilateralBlurFilter::init() {
        if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshBilateralBlur)) {
            ...
            if (loc < 0)
                return false;
            const float factors[9] = {0.05f, 0.09f, 0.12f, 0.15f, 0.18f, 0.15f, 0.12f, 0.09f,
                                      0.05f};
    ...
    ConstString CGEBilateralBlurBetterFilter::paramBlurRadiusName = "samplerRadius";
    ...
    void
    CGEBilateralWrapperFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
