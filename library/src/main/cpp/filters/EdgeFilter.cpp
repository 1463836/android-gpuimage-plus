                                     ...
                                     (
                                             ...
                                             void main() {
                                                 vec4 src = texture2D(inputImageTexture,
                                                                      textureCoordinate);
                                                 vec3 tmpColor = texture2D(inputImageTexture,
                                                                           textureCoordinate +
                                                                           samplerSteps * stride *
                                                                           norm).rgb;
                                                 tmpColor = abs(src.rgb - tmpColor) * 2.0;
                                                 gl_FragColor = vec4(
                                                         mix(src.rgb, tmpColor, intensity), src.a);
                                             }
                                     );

ConstString s_vshEdgeSobel = CGE_SHADER_STRING
(
...
const static char *const s_fshEdgeSobel = CGE_SHADER_STRING_PRECISION_M
                                          (
                                                  ...
                                                  void main() {
                                                      ...
                                                      for (int i = 0; i < 8; ++i) {
                                                          colors[i] = texture2D(inputImageTexture,
                                                                                coords[i]).rgb;
                                                      }
                                                      ...
                                                      vec3 h = -colors[0] - 2.0 * colors[1] -
                                                               colors[2] + colors[5] +
                                                               2.0 * colors[6] + colors[7];
                                                      vec3 v = -colors[0] + colors[2] -
                                                               2.0 * colors[3] + 2.0 * colors[4] -
                                                               colors[5] + colors[7];

                                                      gl_FragColor = vec4(
                                                              mix(src.rgb, sqrt(h * h + v * v),
                                                                  intensity), 1.0);
                                                  }
                                          );
