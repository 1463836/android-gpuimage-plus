...
static ConstString s_fshSketch = CGE_SHADER_STRING_PRECISION_M
                                    (
                                            ...
                                            void main() {
                                                vec4 origin = texture2D(inputImageTexture,
                                                                        textureCoordinate);
                                                vec4 maxValue = texture2D(maxValueTex,
                                                                          textureCoordinate);
                                                ...
                                                gl_FragColor = vec4(
                                                        mix(origin.rgb, vec3(blendColor),
                                                            intensity), origin.a);
                                            });

namespace CGE {
    ConstString CGESketchFilter::paramCacheTextureName = "maxValueTex";
    ConstString CGESketchFilter::paramIntensityName = "intensity";
    ...
    bool CGESketchFilter::init() {
        if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshSketch) &&
            m_maxValueFilter.init()) {
            setIntensity(1.0f);
            m_program.bind();
    ...
    void CGESketchFilter::render2Texture(CGE::ImageHandlerInterface *handler, GLuint srcTexture,
                                         GLuint vertexBufferID) {
        Sizei sz = handler->getOutputFBOSize();

        if (m_textureCache == 0 || sz != m_cacheSize) {
            glDeleteTextures(1, &m_textureCache);
            m_textureCache = genTextureWithBuffer(nullptr, sz.width, sz.height, GL_RGBA,
                                                  GL_UNSIGNED_BYTE);
            m_cacheSize = sz;
        }
