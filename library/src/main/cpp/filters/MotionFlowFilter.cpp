...
ConstString s_fsh = CGE_SHADER_STRING_PRECISION_L
                       (
                               ...
                               void main() {
                                   gl_FragColor =
                                           texture2D(inputImageTexture, textureCoordinate) * alpha;
                               }
                       );

namespace CGE {
    ConstString CGEMotionFlowFilter::paramAlphaName = "alpha";

    CGEMotionFlowFilter::CGEMotionFlowFilter() : m_totalFrames(10), m_delayFrames(0),
                                                 m_delayedFrames(0), m_dAlpha(0.0f) {
    }
...
//        glViewport(0, 0, m_width, m_height);
        ...
        if (m_frameTextures.size() < m_totalFrames) {
            GLuint texID = genTextureWithBuffer(nullptr, m_width, m_height, GL_RGBA,
                                                GL_UNSIGNED_BYTE);
    ...
    void CGEMotionFlowFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
