...
static ConstString s_fshSelectiveColor = CGE_SHADER_STRING_PRECISION_H
                                            (
                                                    ...
                                                    vec4 getFac(vec4 facPre, vec4 facNex, float h,
                                                                float s, float v) {
                                                        vec4 ret = mix(facPre, facNex,
                                                                       h); // facNex * h + facPre - h * facPre = (facNex - facPre) * h + facPre
                                                    ...
                                                    vec3 adjColor(vec3 src, float a, vec4 fac) {
                                                        vec3 tfac =
                                                                fac.xyz * vec3(fac.w, fac.w, fac.w);

                                                        tfac = min(tfac, vec3(2.0, 2.0, 2.0));

                                                        vec3 tfc1 = clamp(
                                                                tfac - vec3(1.0, 1.0, 1.0), 0.0,
                                                                0.5);
                                                        vec3 tfc2 = max(tfac - vec3(1.5, 1.5, 1.5),
                                                                        0.0);
                                                    ...
                                                    vec3 hsvAdjust(vec3 src) {
                                                        ...
                                                        vec4 fac = getFac(color1, color2, s / d, d,
                                                                          temp.x);
...
namespace CGE {
    ConstString CGESelectiveColorFilter::paramName = "intensity";
