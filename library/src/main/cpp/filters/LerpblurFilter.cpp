...
static ConstString s_fshScale = CGE_SHADER_STRING_PRECISION_M
                                   (
...
namespace CGE {
    ...
    void CGELerpblurFilter::_genMipmaps(int width, int height) {
        ...
        if(textureType == GL_FALSE)
        {
            ...
            if(cgeCheckGLExtension("EXT_color_buffer_half_float"))
            {
                ...
                logi("###Lerp blur: EXT_color_buffer_half_float used!!\n");
            }
#endif
        }
        ...
        for (int i = 0; i != MAX_LERP_BLUR_INTENSITY; ++i) {
            Sizei sz(_calcLevel(width, i + 2), _calcLevel(height, i + 2));
            if (sz.width < 1)
                sz.width = 1;
            if (sz.height < 1)
                sz.height = 1;
            glBindTexture(GL_TEXTURE_2D, texIDs[i]);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sz.width, sz.height, 0, GL_RGBA, textureType,
                         0);
    ...
    void CGELerpblurFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
        ...
        if (m_texCache[0].texID == 0 || m_cacheTargetSize.width != sz.width ||
            m_cacheTargetSize.height != sz.height || m_isBaseChanged) {
            _genMipmaps(sz.width, sz.height);
            m_cacheTargetSize = sz;
            m_isBaseChanged = false;
            logi("CGELerpblurFilter::render2Texture - Base Changing!\n");
        }
