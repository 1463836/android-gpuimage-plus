...
static ConstString s_vshMapingBuffered = CGE_SHADER_STRING
(
...
static ConstString s_fshMapingBuffered = CGE_SHADER_STRING_PRECISION_M
                                            (
                                                    varying vec2 textureCoordinate;
...
namespace CGE {
    class CGEColorMappingFilterBuffered_Area : public CGEColorMappingFilter {
        CGEColorMappingFilterBuffered_Area() : m_mappingVertBuffer(0), m_mappingTexVertBuffer(0),
                                               m_drawer(nullptr) {}
    ...
    public:
        ...
        void render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
                            GLuint vertexBufferID) {
            assert(m_mappingAreas.size() > 0 && m_cacheBufferData.size() >=
                                                m_texUnitResolution.width *
                                                m_texUnitResolution.height &&
                   m_texVertBufferData.size() >=
                   m_texUnitResolution.width * m_texUnitResolution.height * 6);
            ...
            glFinish();
            glReadPixels(0, 0, m_texUnitResolution.width, m_texUnitResolution.height, GL_RGBA,
                         GL_UNSIGNED_BYTE, m_cacheBufferData.data());
            ...
            glBufferSubData(GL_ARRAY_BUFFER, 0,
                            m_texVertBufferData.size() * sizeof(m_texVertBufferData[0]),
        ...
        void setupMapping(GLuint mappingTex, int texWidth, int texHeight, int texUnitWidth,
                          int texUnitHeight) {
            CGEColorMappingFilter::setupMapping(mappingTex, texWidth, texHeight, texUnitWidth,
                                                texUnitHeight);
            ...
            glBindBuffer(GL_ARRAY_BUFFER, m_mappingVertBuffer);
            glBufferData(GL_ARRAY_BUFFER, vertBuf.size() * sizeof(vertBuf[0]), vertBuf.data(),
                         GL_STATIC_DRAW);
            m_drawCount = (int) vertBuf.size();

            glBindBuffer(GL_ARRAY_BUFFER, m_mappingTexVertBuffer);
            glBufferData(GL_ARRAY_BUFFER,
                         m_texVertBufferData.size() * sizeof(m_texVertBufferData[0]), nullptr,
                         GL_STREAM_DRAW);
...
// 		}
    ...
    void CGEColorMappingFilter::setupMapping(GLuint mappingTex, int texWidth, int texHeight,
                                             int texUnitWidth, int texUnitHeight) {
