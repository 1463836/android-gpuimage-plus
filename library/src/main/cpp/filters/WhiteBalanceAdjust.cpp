...
const static char *const s_fshWhiteBalance = CGE_SHADER_STRING_PRECISION_H
                                             (
                                                     ...
                                                     vec3 whiteBalance(vec3 src, float temp,
                                                                       float tint) {
                                                         ...
                                                         if (temp < 4.0)
                                                             xD = 0.27475 / (temp * temp * temp) -
                                                                  0.98598 / (temp * temp) +
                                                                  1.17444 / temp + 0.145986;
                                                         else if (temp < 7.0)
                                                             xD = -4.6070 / (temp * temp * temp) +
                                                                  2.9678 / (temp * temp) +
                                                                  0.09911 / temp + 0.244063;
                                                         else
                                                             xD = -2.0064 / (temp * temp * temp) +
                                                                  1.9018 / (temp * temp) +
                                                                  0.24748 / temp + 0.237040;

                                                         float yD =
                                                                 -3.0 * xD * xD + 2.87 * xD - 0.275;
                                                         ...
                                                         color.r = X * 3.24074 - 1.53726 -
                                                                   Z * 0.498571;
                                                         color.g = -X * 0.969258 + 1.87599 +
                                                                   Z * 0.0415557;
                                                         color.b = X * 0.0556352 - 0.203996 +
                                                                   Z * 1.05707;

                                                         color.g /= tint;

                                                         color /= max(max(color.r, color.g),
                                                                      color.b);
                                                         color = 1.0 / color;
                                                         color /=
                                                                 color.r * 0.299 + color.g * 0.587 +
                                                                 color.b * 0.114;
                                                     ...
                                                     void main() {
                                                         vec4 src = texture2D(inputImageTexture,
                                                                              textureCoordinate);
                                                         src.rgb = dispatch(
                                                                 whiteBalance(src.rgb, temperature,
                                                                              tint));
                                                         gl_FragColor = src;
                                                     }
                                             );
...
namespace CGE {
    ConstString CGEWhiteBalanceFilter::paramTemperatureName = "temperature";
    ConstString CGEWhiteBalanceFilter::paramTintName = "tint";
    ...
    ConstString CGEWhiteBalanceFastFilter::paramBalanceName = "balance";
    ...
    void CGEWhiteBalanceFastFilter::setTempAndTint(float temp, float tint) {
        ...
        temp /= 1000.0f;
        if (temp < 4.0f)
            xD = 0.27475f / (temp * temp * temp) - 0.98598f / (temp * temp) + 1.17444f / temp +
                 0.145986f;
        else if (temp < 7.0f)
            xD = -4.6070f / (temp * temp * temp) + 2.9678f / (temp * temp) + 0.09911f / temp +
                 0.244063f;
        else
            xD = -2.0064f / (temp * temp * temp) + 1.9018f / (temp * temp) + 0.24748f / temp +
                 0.237040f;
