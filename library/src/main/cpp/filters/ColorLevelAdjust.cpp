...
const static char *const s_fshColorLevel = CGE_SHADER_STRING_PRECISION_M
                                           (
                                                   ...
                                                   vec3 levelFunc(vec3 src, vec2 colorLevel) {
                                                       return clamp((src - colorLevel.x) /
                                                                    (colorLevel.y - colorLevel.x),
                                                                    0.0, 1.0);
                                                   }

                                                   vec3
                                                   gammaFunc(vec3 src, float value) //value: 0~1
                                                   {
                                                       return clamp(pow(src, vec3(value)), 0.0,
                                                                    1.0);
                                                   }
...
namespace CGE {
    ConstString CGEColorLevelFilter::paramLevelName = "colorLevel";
    ConstString CGEColorLevelFilter::paramGammaName = "gamma";
