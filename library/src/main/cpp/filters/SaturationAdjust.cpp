...
//		S = (maxc - minc) / (2.0 - maxc - minc);
                                                       ...
                                                       S = mix(temp1 / (2.0 - maxc - minc),
                                                               temp1 / (maxc + minc), step(L, 0.5));
...
// 		return vec3(src.z, src.z, src.z);
                                                       float q = (src.z < 0.5) ? src.z *
                                                                                 (1.0 + src.y) : (
                                                                         src.z + src.y -
                                                                         (src.y * src.z));
                                                       float p = 2.0 * src.z - q;

                                                       vec3 dst = vec3(src.x + 0.333, src.x,
                                                                       src.x - 0.333);
...
// 	else dst.b = p;
                                                       ...
                                                       vec3 weight2 =
                                                               weight_neg * step(dst, vec3(0.5));
                                                       vec3 weight2_neg =
                                                               weight_neg * (1.0 - weight2);

                                                       vec3 weight3 = weight2_neg *
                                                                      step(dst, vec3(2.0 / 3.0));
                                                   ...
                                                   vec3 adjustColor(vec3 src, float h, float s,
                                                                    float l) //hue should be positive
                                                   {
                                                       src = RGB2HSL(src);
                                                       src.x += h;
                                                       src.y *= 1.0 + s;
                                                   ...
                                                   void main() {
                                                       vec4 src = texture2D(inputImageTexture,
                                                                            textureCoordinate);
                                                       src.rgb = adjustColor(src.rgb, hue,
...
const static char *const s_fshHSVAdjust = CGE_SHADER_STRING_PRECISION_M
                                          (
                                                  ...
                                                  vec3 hsvAdjust(vec3 src, vec3 color1,
                                                                 vec3 color2) //color1:red green blue, color2: magenta yellow cyan
                                                  {
                                                      ...
                                                      if (fmax == src.r) {
                                                          if (src.g >= src.b) {
                                                              ...
                                                              src.g = hsv.z *
                                                                      (1.0 - hsv.y + hsv.y * hsv.x);
                                                          } else {
                                                              hsv.x = (src.r - src.b) / fdelta;
                                                              fs_off = (color1.r - color2.r) *
                                                                       hsv.x + color2.r;
                                                              //saturation adjust
                                                              hsv.y = hsv.y * (1.0 + fs_off);
                                                              clamp(hsv.y, 0.0, 1.0);
                                                  ...
                                                  void main() {
                                                      vec4 src = texture2D(inputImageTexture,
                                                                           textureCoordinate);
                                                      src.rgb = hsvAdjust(src.rgb, vColor1,
                                                                          vColor2);
                                                      gl_FragColor = src;
                                                  }
                                          );
...
namespace CGE {
    ConstString CGESaturationHSLFilter::paramSaturationName = "saturation";
    ConstString CGESaturationHSLFilter::paramHueName = "hue";
    ConstString CGESaturationHSLFilter::paramLuminanceName = "luminance";
    ...
    ConstString CGESaturationFilter::paramIntensityName = "intensity";
    ...
    ConstString CGESaturationHSVFilter::paramColor1 = "vColor1";
    ConstString CGESaturationHSVFilter::paramColor2 = "vColor2";

    bool CGESaturationHSVFilter::init() {
        return initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshHSVAdjust);
    }

    void CGESaturationHSVFilter::setAdjustColors(float red, float green, float blue, float magenta,
                                                 float yellow, float cyan) {
        m_program.bind();
