...
static ConstString s_fshPolarPixellate = CGE_SHADER_STRING_PRECISION_M
                                            (
                                                    ...
                                                    void main() {
                                                        vec2 normCoord =
                                                                2.0 * textureCoordinate - 1.0;
                                                        vec2 normCenter = 2.0 * center - 1.0;

                                                        normCoord -= normCenter;

                                                        float r = length(
                                                                normCoord); // to polar coords
                                                        float phi = atan(normCoord.y,
                                                                         normCoord.x); // to polar coords
...
namespace CGE {
    ConstString CGEPolarPixellateFilter::paramCenter = "center";
    ConstString CGEPolarPixellateFilter::paramPixelSize = "pixelSize";
