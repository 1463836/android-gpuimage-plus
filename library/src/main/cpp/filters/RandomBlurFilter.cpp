...
const static char *const s_fshRandomBlur = CGE_SHADER_STRING_PRECISION_M
                                           (
                                                   ...
                                                   void main() {
                                                       ...
                                                       for (float fy = -samplerRadius;
                                                            fy <= samplerRadius; ++fy) {
                                                           for (float fx = -samplerRadius;
                                                                fx <= samplerRadius; ++fx) {
                                                               float dis = length(vec2(fx, fy) *
                                                                                  samplerSteps) *
                                                                           blurSamplerScale;
                                                               vec2 coord = dis * vec2(cos(dir),
                                                                                       sin(dir));
                                                               sumColor += texture2D(
                                                                       inputImageTexture,
                                                                       textureCoordinate +
                                                       ...
                                                       vec3 dst = mix(src.rgb,
                                                                      sumColor / samplingCount,
                                                                      intensity);
...
namespace CGE {
    ConstString CGERandomBlurFilter::paramIntensity = "intensity";
    ConstString CGERandomBlurFilter::paramSamplerScale = "blurSamplerScale";
    ConstString CGERandomBlurFilter::paramSamplerRadius = "samplerRadius";
