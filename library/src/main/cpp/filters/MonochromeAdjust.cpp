...
const static char *const s_fshMonochrome = CGE_SHADER_STRING_PRECISION_H
                                           (
                                                   ...
                                                   void main() {
                                                       ...
                                                       ratioMax, ratioMin;
                                                       ratioMax.xy = vec2(
                                                               equal(src.rg, vec2(maxc)));
                                                       float max_neg = 1.0 - ratioMax.x;
                                                       ratioMax.y *= max_neg;
                                                       ratioMax.z = (1.0 - ratioMax.y) * max_neg;

                                                       vec3 compMax =
                                                               vec3(red, green, blue) * ratioMax;

                                                       ratioMin.xy = vec2(
                                                               equal(src.rg, vec2(minc)));
                                                       float min_neg = 1.0 - ratioMin.x;
                                                       ratioMin.y *= min_neg;
                                                       ratioMin.z = (1.0 - ratioMin.y) * min_neg;

                                                       vec3 compMaxMid =
                                                               vec3(cyan, magenta, yellow) *
                                                               ratioMin;

                                                       float total =
                                                               (compMax.x + compMax.y + compMax.z) *
                                                               (maxc - midc) +
                                                               (compMaxMid.x + compMaxMid.y +
                                                                compMaxMid.z) * (midc - minc) +
                                                               minc;

                                                       gl_FragColor = vec4(total, total, total,
                                                                           1.0);
                                                   }
                                           );

namespace CGE {
    ConstString CGEMonochromeFilter::paramRed = "red";
    ConstString CGEMonochromeFilter::paramGreen = "green";
    ConstString CGEMonochromeFilter::paramBlue = "blue";
    ConstString CGEMonochromeFilter::paramCyan = "cyan";
    ConstString CGEMonochromeFilter::paramMagenta = "magenta";
    ConstString CGEMonochromeFilter::paramYellow = "yellow";
