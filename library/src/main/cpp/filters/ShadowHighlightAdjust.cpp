...
const static char *const s_fshShadowHighlight = CGE_SHADER_STRING_PRECISION_H
                                                (
                                                        ...
                                                        vec3 highlightAndShadow(vec3 src, float l,
                                                                                float d) //l for highlight, d for shadow
                                                        {
                                                            vec3 src2 = src * src;
                                                            vec3 src3 = src2 * src;
                                                            vec3 color1 = 4.0 * (d - 1.0) * src3 +
                                                                          4.0 * (1.0 - d) * src2 +
                                                                          d * src;
                                                            vec3 color2 = 4.0 * (l - 1.0) * src3 +
                                                                          8.0 * (1.0 - l) * src2 +
                                                                          (5.0 * l - 4.0) * src +
                                                                          1.0 - l;
                                                        ...
                                                        void main() {
                                                            vec4 src = texture2D(inputImageTexture,
                                                                                 textureCoordinate);
                                                            src.rgb = highlightAndShadow(
                                                                    src.rgb * src.a, highlights,
                                                                    shadows);
                                                            gl_FragColor = src;
                                                        }
                                                );
...
namespace CGE {
    ConstString CGEShadowHighlightFilter::paramShadowName = "shadows";
    ConstString CGEShadowHighlightFilter::paramHighlightName = "highlights";
    ...
    void CGEShadowHighlightFastFilter::setShadowAndHighlight(float shadow, float highlight) {
        ...
        for (std::vector<float>::size_type t = 0; t != sz; ++t) {
            ...
            if (v < 0.5)
                ...
                m_curveRGB[t] = 4.0f * (l - 1.0f) * v * v * v + 8.0f * (1.0f - l) * v * v +
                                (5.0f * l - 4.0f) * v + 1.0f - l;
        }
        assignCurveArray();
    }
