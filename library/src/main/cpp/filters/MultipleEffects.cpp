...
static ConstString s_fshHTFilterMix = CGE_SHADER_STRING_PRECISION_M
                                         (
                                                 ...
                                                 void main() {
                                                     vec4 src = texture2D(inputImageTexture,
                                                                          textureCoordinate);
                                                     vec4 origin = texture2D(originImageTexture,
                                                                             textureCoordinate);
...
namespace CGE {
    ...
    ConstString CGEMutipleMixFilter::paramIntensityName = "intensity";
    ConstString CGEMutipleMixFilter::paramOriginImageName = "originImageTexture";
    ...
    void CGEMutipleMixFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
    ...
    CGEMutipleEffectFilter::~CGEMutipleEffectFilter() {
        ...
        logi("CGEMutipleEffectFilter Release...\n");
    }

    void CGEMutipleEffectFilter::clearFilters() {
        for (std::vector<ImageFilterInterface *>::iterator iter = m_vecFilters.begin();
    ...
    GLuint CGEMutipleEffectFilter::loadResources(const char *textureName, int *width, int *height) {
        ...
        void *pArg;
        if (loadFunc == nullptr ||
            (pArg = loadFunc(textureName, &bufferData, &w, &h, &fmt, loadArg)) == nullptr) {
            loge("Load texture %s failed!\n", textureName);
            return 0;
        }

        GLenum dataFmt, channelFmt;
        getDataAndChannelByFormat(fmt, &dataFmt, &channelFmt, nullptr);
        GLuint texture = genTextureWithBuffer(bufferData, w, h, channelFmt, dataFmt);
        if (width != nullptr)
            *width = w;
        if (height != nullptr)
            *height = h;
        ...
        while (*ptr != '\0') {
            ...
            *pBuffer = '\0';
            ...
            if (strcmp(buffer, "blend") == 0) {
                ...
                loge("指令未被解析(Invalid parameters):%s", ptr);
            }
        }

        if (m_vecFilters.empty()) {
            loge("特效指令 \"%s\" 无法生成任何特效!\n", effectString);
            return false;
        }
    ...
    void
    CGEMutipleEffectFilter::render2Texture(ImageHandlerInterface *fastFrameHandler, GLuint srcTexture,
                                           GLuint vertexBufferID) {
        if (m_vecFilters.empty() || m_mixFilter.noIntensity() CGE_LOG_CODE(|| m_isWrapper)) {
            CGE_LOG_CODE
            (
                    if (m_vecFilters.empty()) {
                        loge("CGEMutipleEffectFilter::render2Texture did nothing!\n");
                    }
            ...
            fastFrameHandler->swapBufferFBO();
            return;
        }

        const auto &sz = fastFrameHandler->getOutputFBOSize();
        ...
        if (needMix) {
            if (m_texCache == 0 || sz != m_currentSize) {
                m_currentSize = sz;
                glDeleteTextures(1, &m_texCache);
                m_texCache = genTextureWithBuffer(nullptr, m_currentSize.width,
                                                  m_currentSize.height, GL_RGBA,
                                                  GL_UNSIGNED_BYTE, 4, 0, GL_NEAREST,
                                                  GL_CLAMP_TO_EDGE);
            }
            fastFrameHandler->copyLastResultTexture(m_texCache);
        }

        std::vector<ImageFilterInterface *>::iterator iter = m_vecFilters.begin();
        for (;;) {
            glBindBuffer(GL_ARRAY_BUFFER, vertexBufferID);
            (*iter)->render2Texture(fastFrameHandler, fastFrameHandler->getBufferTextureID(), vertexBufferID);
        ...
        if (needMix) {
            fastFrameHandler->swapBufferFBO();
            glBindBuffer(GL_ARRAY_BUFFER, vertexBufferID);
            m_mixFilter.render2Texture(fastFrameHandler, m_texCache, vertexBufferID);
        }
    }

    std::vector<ImageFilterInterface *> CGEMutipleEffectFilter::getFilters(bool bMove) {
        if (!bMove)
            return m_vecFilters;
