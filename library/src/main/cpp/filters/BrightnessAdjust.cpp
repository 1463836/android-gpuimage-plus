...
namespace CGE {
    ConstString CGEBrightnessFilter::paramName = "intensity";
    ...
    void CGEBrightnessFilter::render2Texture(ImageHandlerInterface *handler, GLuint srcTexture,
    ...
    void CGEBrightnessFastFilter::setIntensity(float value) {
        ...
        for (std::vector<float>::size_type t = 0; t != sz; ++t) {
            ...
            m_curveRGB[t] = 1.0f - v - (fac / SQRT2) + iSign * sqrtf(1.0f - SQRT2 * fac +
                                                                     2.0f * SQRT2 * v * fac +
                                                                     0.5f * fac * fac);
        }
        assignCurveArray();
