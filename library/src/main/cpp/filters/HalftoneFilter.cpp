...
static ConstString s_fshHalftone = CGE_SHADER_STRING_PRECISION_H
                                      (
...
//const vec3 weight = vec3(0.2125, 0.7154, 0.0721);
                                              ...
                                              void main() {
                                                  vec2 samplePos = textureCoordinate -
                                                                   mod(textureCoordinate,
                                                                       dotPercent) +
                                                                   0.5 * dotPercent;
                                                  vec2 coordToUse = vec2(textureCoordinate.x,
                                                                         (textureCoordinate.y -
                                                                          0.5) * aspectRatio + 0.5);
                                                  vec2 adjustedPos = vec2(samplePos.x,
                                                                          (samplePos.y - 0.5) *
                                                                          aspectRatio + 0.5);
                                                  ...
                                                  vec4 color = texture2D(inputImageTexture,
                                                                         samplePos);
                                                  vec3 dotScaling =
                                                          1.0 - color.rgb;//dot(color.rgb, weight);
                                                  vec3 presenceDot = 1.0 - step(dis, dotPercent.x *
                                                                                     dotScaling *
                                                                                     0.5);
...
namespace CGE {
    ConstString CGEHalftoneFilter::paramAspectRatio = "aspectRatio";
    ConstString CGEHalftoneFilter::paramDotPercent = "dotPercent";
    ...
    void
    CGEHalftoneFilter::render2Texture(CGE::ImageHandlerInterface *handler, GLuint srcTexture,
